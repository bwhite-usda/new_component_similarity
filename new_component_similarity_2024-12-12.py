# This is new_component_similarity_2024-12-12.py
import pandas as pd
...
# 4. Create a DataFrame from the filtered results
filtered_df = pd.DataFrame(filtered_data)

# Dynamically define output columns based on available fields
output_columns = [
    ...
    "Linkage mandated by what US Code or OMB policy?",  # Optional column
    "Enabling Component URL", "Dependent Component URL",
    "Enabling Source Agency", "Dependent Source Agency", "Similarity"
]

# Retain only existing columns to avoid KeyError
existing_columns = [col for col in output_columns if col in filtered_df.columns]
filtered_df = filtered_df[existing_columns]

# 5. Save the filtered results to an Excel file
